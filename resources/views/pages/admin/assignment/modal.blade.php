                ...
                <div class="col d-flex flex-column">
                    ...
                    <div class="modal-footer d-flex justify-content-between align-items-center p-2">
                        ...
                        <button type="button" class="btn btn-light btn-sm like-btn" data-assignment-id="{{ $row->id }}">
                            <i class="bi {{ $row->isLikedByUser() ? 'bi-heart-fill' : 'bi-heart' }} fs-1 text-danger"></i>{{ $row->likeCount() }}
                        </button>

                        <!-- Form -->
                        <form class="d-flex ms-2" style="width: 85%;" action="{{route('assignment.comment')}}" method="POST">
                            @csrf
